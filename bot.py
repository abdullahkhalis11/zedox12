...
def main_menu(uid):
    ...
    kb.row("⭐ BUY VIP","🎁 REFERRAL")
    kb.row("👤 ACCOUNT","🆔 CHAT ID","🏆 Redeem")
    # Admin panel button for admin only
    if uid == ADMIN_ID:
...
# =========================
# CATEGORY HANDLER (FREE, VIP, APPS)
# =========================
...
def category_handler(m):
    ...
    cat = cat_map[m.text]
    bot.send_message(uid, f"📂 {m.text}", reply_markup=folder_buttons(cat, page=0))

# =========================
# CATEGORY BUTTON PAGINATION
# =========================
def folder_buttons(category, page=0, per_page=8):
    ...
    if not items:
        kb.add(InlineKeyboardButton("No folders available", callback_data="none"))
        return kb
    ...
    for name, info in items[start:end]:
        ...
        label = f"{name} - {price} pts" if price>0 else f"{name} - FREE"
        kb.add(InlineKeyboardButton(label, callback_data=f"folder|{category}|{name}"))
...
def folder_callback(c):
    ...
    if data[0]=="folder":
        ...
        user = User(uid)
        if price>0 and not user.is_vip() and uid != ADMIN_ID:
            bot.answer_callback_query(c.id,"💎 VIP required or insufficient points.")
            return
...
def referral_handler(m):
    ...
    users = load("users.json")
    referred = sum(1 for u in users.values() if u.get("ref")==str(uid))
...
# VIP BUY & POINTS
# =========================
@bot.message_handler(func=lambda m: m.text in ["💰 POINTS","⭐ BUY VIP"])
def points_vip_handler(m):
    uid = m.from_user.id
    user = User(uid)
    if m.text=="💰 POINTS":
