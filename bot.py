...
def init_files():
    files = {
        ...
        "config.json": {
            ...
            "ref_reward": 5,
            "notify": True,
            "vip_price": 1000  # Points required to buy VIP
        },
        "codes.json": {}
    }
...
class User:
    ...
    def buy_vip(self):
        ...
        vip_price = cfg.get("vip_price", 1000)
        
        if self.points() >= vip_price and not self.is_vip():
            self.add_points(-vip_price)
            self.make_vip()
            return True, vip_price
        return False, vip_price
...
def admin_menu():
    ...
    kb.row("💰 Set VIP Price", "🏆 Generate Codes")
    kb.row("📤 Broadcast")
...
def set_vip_price_start(m):
    msg = bot.send_message(m.from_user.id, "💰 Enter the number of points required to buy VIP:\n\nCurrent price: **{}** points".format(load("config.json").get("vip_price", 1000)), parse_mode="Markdown")
...
def save_vip_price(m):
    try:
        ...
        bot.send_message(m.from_user.id, f"✅ VIP price updated to **{price} points**!", parse_mode="Markdown")
    except:
...
# =========================
# ZEDOX BOT - PART 3
# User Panel + Start + Folders + Redeem + Referral + Buy VIP
# =========================
...
def open_folder(c):
    ...
    if cat == "vip" and not user.is_vip():
        ...
        vip_price = cfg.get("vip_price", 1000)
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("⭐ Buy VIP with Points", callback_data="buy_vip"))
        bot.send_message(uid, f"{cfg['vip_msg']}\n\n💰 **VIP Price:** {vip_price} points\n\nUse the button below to buy VIP:", reply_markup=kb, parse_mode="Markdown")
        return
...
def buy_vip_callback(c):
    ...
    success, price = user.buy_vip()
    
    if success:
        bot.answer_callback_query(c.id, f"🎉 Congratulations! You are now VIP!", show_alert=True)
        ...
        bot.send_message(uid, "💎 You can now access all VIP content!", reply_markup=main_menu(uid))
    else:
        bot.answer_callback_query(c.id, f"❌ You need {price} points to buy VIP! You have {user.points()} points.", show_alert=True)
...
def user_commands(m):
    ...
    if t == "💰 POINTS":
        ...
        if user.is_vip():
            ...
            bot.send_message(
                uid, 
                f"💎 **VIP Membership**\n\n✨ **Benefits:**\n• Access to all VIP methods\n• Premium content\n• Exclusive features\n\n💰 **Price:** {vip_price} points\n\n📊 **Your Points:** {user.points()}\n\nClick the button below to purchase VIP with your points:",
                reply_markup=kb,
                parse_mode="Markdown"
            )
        ...
        if not user.is_vip():
            account_text += f"\n💎 VIP Price: {vip_price} points\n"
            account_text += f"Points needed: **{max(0, vip_price - user.points())}** points"
