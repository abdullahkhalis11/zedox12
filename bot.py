...
class ForceJoin:
    ...
    def join_buttons(self):
        ...
        kb.add(InlineKeyboardButton("✅ I've Joined", callback_data="recheck"))
        return kb
...
def set_welcome_step(m):
    msg = bot.send_message(m.from_user.id,"✏️ Send the new welcome message (supports photo/video later):")
...
# =========================
# FORCE JOIN CHANNEL MANAGEMENT (FIXED)
# =========================
...
def remove_force_channel_step2(m):
    ...
    if channel in config["force_channels"]:
        ...
        save("config.json", config)
        bot.send_message(m.from_user.id,f"✅ Removed force join channel `{channel}`.")
    else:
...
def delete_folder_step1(m):
    ...
    for cat in ["free", "vip", "apps", "courses"]:
        if db[cat]:
            kb.add(InlineKeyboardButton(f"📂 {cat.upper()}", callback_data=f"del_cat|{cat}"))
    if not kb.keyboard:
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("del_cat") and is_admin(c.from_user.id))
def delete_folder_step2(c):
    cat = c.data.split("|")[1]
    db = load("db.json")
    kb = InlineKeyboardMarkup()
    for folder in db[cat].keys():
        kb.add(InlineKeyboardButton(f"🗑 {folder}", callback_data=f"del_folder|{cat}|{folder}"))
    kb.add(InlineKeyboardButton("🔙 Back", callback_data="back_admin"))
    bot.edit_message_text(f"Select folder to delete from {cat.upper()}:", c.from_user.id, c.message.message_id, reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data.startswith("del_folder") and is_admin(c.from_user.id))
def delete_folder_step3(c):
...
def edit_price_step1(m):
    ...
    for cat in ["free", "vip", "apps", "courses"]:
        if db[cat]:
            kb.add(InlineKeyboardButton(f"📂 {cat.upper()}", callback_data=f"price_cat|{cat}"))
    if not kb.keyboard:
        bot.send_message(m.from_user.id, "❌ No folders available to edit.")
        return
    bot.send_message(m.from_user.id, "Select category to edit folder price:", reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data.startswith("price_cat") and is_admin(c.from_user.id))
def edit_price_step2(c):
    cat = c.data.split("|")[1]
    db = load("db.json")
    kb = InlineKeyboardMarkup()
    for folder in db[cat].keys():
        kb.add(InlineKeyboardButton(f"💰 {folder}", callback_data=f"price_folder|{cat}|{folder}"))
    kb.add(InlineKeyboardButton("🔙 Back", callback_data="back_admin"))
    bot.edit_message_text(f"Select folder to edit price from {cat.upper()}:", c.from_user.id, c.message.message_id, reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data.startswith("price_folder") and is_admin(c.from_user.id))
def edit_price_step3(c):
    _, cat, folder = c.data.split("|")
    msg = bot.send_message(c.from_user.id, f"✏️ Send new price for '{folder}' (0 for free):")
    bot.register_next_step_handler(msg, lambda m: edit_price_step4(m, cat, folder, c.message.message_id))

def edit_price_step4(m, cat, folder, msg_id):
    try:
        price = int(m.text)
...
def generate_codes_step2(m):
    try:
        points = int(m.text)
        msg = bot.send_message(m.from_user.id,"✏️ Send number of codes to generate:")
...
# =========================
# ZEDOX BOT - PART 3
# Upload System, Redeem Codes & Broadcast
# =========================
...
def open_callback(c):
    ...
    # Price check for all categories (courses and apps also require points if not VIP)
    ...
    if load("config.json")["notify"] and sent_count > 0:
        bot.send_message(c.from_user.id,f"✅ Sent {sent_count} file(s) from `{name}`")

# =========================
# FORCE JOIN RECHECK (FIXED)
# =========================
@bot.callback_query_handler(func=lambda c: c.data=="recheck")
def recheck_join(c):
    uid = c.from_user.id
    if force.check(uid):
        bot.edit_message_text("✅ Access granted! Welcome!", uid, c.message.message_id)
        ...
        bot.edit_message_text("🚫 ACCESS DENIED! Please join all channels:", uid, c.message.message_id, reply_markup=force.join_buttons())

# =========================
# BACK TO ADMIN CALLBACK
# =========================
@bot.callback_query_handler(func=lambda c: c.data=="back_admin" and is_admin(c.from_user.id))
def back_to_admin(c):
    bot.edit_message_text("🛠️ *Admin Panel*\nSelect an option below:", c.from_user.id, c.message.message_id, reply_markup=None)
