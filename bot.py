# =========================
# ZEDOX BOT - ULTIMATE EDITION (FULLY FUNCTIONAL)
# Professional Telegram File Sharing Bot with VIP System
# =========================

import telebot
from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton, ReplyKeyboardMarkup, KeyboardButton
import json, os, time, random, string, threading, datetime
...
def get_kb(cat, page=0):
    ...
    for name, d in items:
        ...
        if price > 0:
            txt = f"📁 {name} | 💰 {price} pts | 👁 {downloads}"
        else:
            txt = f"📁 {name} | 🆓 FREE | 👁 {downloads}"
...
def main_menu(uid):
    ...
    btn1 = KeyboardButton("📂 FREE METHODS")
    btn2 = KeyboardButton("💎 VIP METHODS")
    btn3 = KeyboardButton("📱 PREMIUM APPS")
    btn4 = KeyboardButton("🎓 PREMIUM COURSES")
    
    kb.add(btn1, btn2)
    kb.add(btn3, btn4)
    
    btn5 = KeyboardButton("💰 POINTS")
    btn6 = KeyboardButton("⭐ BUY VIP")
    btn7 = KeyboardButton("🎁 REFERRAL")
    btn8 = KeyboardButton("👤 ACCOUNT")
    
    kb.add(btn5, btn6)
    kb.add(btn7, btn8)
    
    btn9 = KeyboardButton("📊 STATS")
    btn10 = KeyboardButton("🏆 REDEEM")
    btn11 = KeyboardButton("🔥 DAILY BONUS")
...
def admin_menu():
    ...
    kb.add("💰 Set Daily Bonus", "📊 Bot Stats")
    kb.add("➕ Add Channel", "➖ Remove Channel")
    kb.add("🔧 Maintenance Mode", "👥 Users List")
    kb.add("💎 Make VIP", "💰 Add Points")
    kb.add("❌ Exit Admin")
...
@bot.message_handler(func=lambda m: m.text == "💰 Set Daily Bonus" and is_admin(m.from_user.id))
def set_bonus(m):
    msg = bot.send_message(m.from_user.id, "💰 Enter daily bonus points:")
    bot.register_next_step_handler(msg, save_bonus)

def save_bonus(m):
    try:
        points = int(m.text)
        cfg = load("config.json")
        cfg["daily_bonus"] = points
        save("config.json", cfg)
        bot.send_message(m.from_user.id, f"✅ Daily bonus set to {points} points!")
    except:
...
@bot.message_handler(func=lambda m: m.text == "💎 Make VIP" and is_admin(m.from_user.id))
def make_vip_start(m):
    msg = bot.send_message(m.from_user.id, "👤 Enter user ID:")
    bot.register_next_step_handler(msg, make_vip_user)

def make_vip_user(m):
    uid = m.text
    msg = bot.send_message(m.from_user.id, "📅 Enter VIP duration in days (30 = 1 month):")
    bot.register_next_step_handler(msg, lambda m2: make_vip_days(m2, uid))

def make_vip_days(m, uid):
    try:
        days = int(m.text)
        user = User(uid)
        user.make_vip(days)
        bot.send_message(m.from_user.id, f"✅ User {uid} is now VIP for {days} days!")
    except:
...
def show_folders(m):
    ...
    bot.send_message(uid, f"📁 <b>{m.text}</b>\n\nSelect a folder to view content:", 
                    reply_markup=get_kb(cat, 0), parse_mode="HTML")
...
# =========================
# OPEN FOLDER - FIXED FOR ALL CATEGORIES
# =========================
...
def open_folder(c):
    ...
    if user.is_vip():
        # VIP users get all content free
        bot.answer_callback_query(c.id, "💎 VIP Access - Free!")
    else:
        ...
        # Check points for non-VIP users
        if price > 0:
    ...
    if count > 0:
        ...
        bot.send_message(uid, f"❌ Failed to send files. Please try again later.", parse_mode="HTML")
...
def user_commands(m):
    ...
    if t == "💰 POINTS":
        ...
        bot.send_message(uid,
                        ...
                        f"🤖 Bot Version: {load('config.json').get('bot_version', '3.0')}",
                        parse_mode="HTML")
    ...
    elif t == "⚙️ ADMIN PANEL" and uid == ADMIN_ID:
        admin_panel(m)
