...
def open_admin_panel(m):
    bot.send_message(m.from_user.id,"🛠️ *Admin Panel*\nSelect an option below:", reply_markup=admin_main_menu(m.from_user.id))
...
def exit_admin(m):
    bot.send_message(m.from_user.id,"✅ Exited Admin Panel.", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).row("📂 FREE METHODS","💎 VIP METHODS","📦 PREMIUM APPS","💰 POINTS","⭐ BUY VIP","🎁 REFERRAL","👤 ACCOUNT","🆔 CHAT ID","🏆 Redeem"))
...
def finalize_edit_price(m, cat, name):
    try:
        ...
        bot.send_message(m.from_user.id,f"✅ Price of folder `{name}` updated to {price} points.")
    except:
        bot.send_message(m.from_user.id,"❌ Invalid number. Price not updated.")
        # =========================
# ZEDOX BOT - PART 4
# Folder Navigation, Force Join, Welcome, Referral & Main Menu
# =========================
...
def start_command(m):
    ...
    # Force join check
    if not force.check(uid) and len(force.config["force_channels"])>0:
        bot.send_message(uid,"🔒 You must join the channels first:", reply_markup=force.join_buttons())
        return
...
def chat_id_button(m):
    uid = m.from_user.id
    bot.send_message(uid,f"🆔 Your Chat ID: `{uid}`", reply_markup=main_menu(uid))

# =========================
# SHOW FOLDERS (Paginated)
# =========================
def show_folders(uid, category, page=0, per_page=8):
    ...
    nav_buttons = []
    if page>0:
        nav_buttons.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"folder_page|{category}|{page-1}"))
    if end<len(names):
        nav_buttons.append(InlineKeyboardButton("Next ➡️", callback_data=f"folder_page|{category}|{page+1}"))
    if nav_buttons:
        kb.row(*nav_buttons)
    bot.send_message(uid, f"📂 {category.upper()} FOLDERS (Page {page+1}/{total_pages})", reply_markup=kb)

# =========================
# FOLDER NAVIGATION CALLBACK
# =========================
@bot.callback_query_handler(func=lambda c: c.data.startswith("folder_page"))
def folder_page_callback(c):
    _, category, page = c.data.split("|")
    show_folders(c.from_user.id, category, int(page))
    bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("folder|"))
def folder_open_callback(c):
    _, category, name = c.data.split("|")
    user = User(c.from_user.id)
    folder = fs.db[category][name]
    price = folder["price"]
    if price>0 and not user.is_vip():
        bot.send_message(c.from_user.id, load("config.json").get("vip_msg","💎 Buy VIP to unlock this!"))
    else:
        bot.send_message(c.from_user.id, f"📂 {name} Files:")
...
# =========================
# FREE / VIP / APPS BUTTONS
# =========================
@bot.message_handler(func=lambda m: m.text=="📂 FREE METHODS")
def free_methods(m):
    show_folders(m.from_user.id, "free")

@bot.message_handler(func=lambda m: m.text=="💎 VIP METHODS")
def vip_methods(m):
    show_folders(m.from_user.id, "vip")

@bot.message_handler(func=lambda m: m.text=="📦 PREMIUM APPS")
def apps_methods(m):
    show_folders(m.from_user.id, "apps")

# =========================
# FORCE JOIN RECHECK CALLBACK
# =========================
@bot.callback_query_handler(func=lambda c: c.data=="recheck")
def force_recheck_callback(c):
    uid = c.from_user.id
    if force.check(uid):
        bot.send_message(uid,"✅ You have joined all channels!", reply_markup=main_menu(uid))
    else:
        bot.send_message(uid,"❌ You still need to join all channels.", reply_markup=force.join_buttons())
    bot.answer_callback_query(c.id)
...
def fallback_handler(m):
    bot.send_message(m.from_user.id,"❌ Command not recognized.", reply_markup=main_menu(m.from_user.id))
